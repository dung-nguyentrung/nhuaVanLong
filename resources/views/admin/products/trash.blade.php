    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                <div>
                    <h4 class="mb-3">Danh sách sản phẩm trong thùng rác</h4>
                </div>
                <div>
                    <a href="{{ route('products.index') }}" class="btn btn-primary add-list">Quay lại</a>                                            
                </div>
            </div>
        </div>
        <div class="col-lg-12">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center list-action">
                                @can('product_restore')
                                <form action="{{ route('products.restore',['product' => $item->id]) }}" method="POST">
                                    @csrf
                                    <button class="btn btn-success mr-3" title="Khôi phục" type="submit"><i class="fa fa-trash-restore mr-0"></i></button>
                                </form>                                    
                                @endcan
                                @can('product_delete')
                                <form action="{{ route('products.forceDelete',['product' => $item->id]) }}" method="POST">
                                    @csrf
                                    @method('delete')
