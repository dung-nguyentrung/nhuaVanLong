    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                <div>
                    <h4 class="mb-3">Danh sách chủ đề bài viết</h4>
                </div>
                <div>
                    @can('tag_delete')                        
                    ...
                    @endcan
                    @can('tag_create')
                    <a href="{{ route('tags.create') }}" class="btn btn-primary add-list"><i class="las la-plus mr-3"></i>Thêm chủ đề bài viết</a>                        
                    @endcan
                </div>
            </div>
        </div>
                        ...
                        <td>
                            <div class="d-flex align-items-center list-action">
                                ...
                                <a class="badge badge-info mr-2" data-toggle="tooltip" data-placement="top" title="Xem chi tiết" data-original-title="View"
                                    href="{{ route('tags.show',['tag' => $item->id]) }}"><i class="fa fa-eye mr-0"></i></a>                                    
                                @endcan
                                @can('tag_edit')
                                <a class="badge bg-success mr-2" data-toggle="tooltip" data-placement="top" title="Cập nhật" data-original-title="Edit"
                                    href="{{ route('tags.edit',['tag' => $item->id]) }}"><i class="fa fa-pen mr-0"></i></a>                                    
                                @endcan
                                @can('tag_delete')
                                <form action="{{ route('tags.destroy',['tag' => $item->id]) }}" method="POST">
                                    @csrf
                                    @method('delete')
