        ...
        <div class="col-lg-12">
            <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                ...
                <div>
                    ...
                    <a href="#" id="deleteAllPostCategorySelected" class="btn btn-danger add-list"><i class="las la-trash"></i>Xóa lựa chọn</a>
                    @endcan
                    @can('post_category_create')
                    <a href="{{ route('post_categories.create') }}" class="btn btn-primary add-list"><i class="las la-plus mr-3"></i>Thêm danh mục bài viết</a>                        
                    @endcan
                </div>
            </div>
        </div>
                        ...
                        <td>
                            <div class="d-flex align-items-center list-action">
                                ...
                                <a class="badge badge-info mr-2" data-toggle="tooltip" data-placement="top" title="Xem chi tiết" data-original-title="View"
                                    href="{{ route('post_categories.show',['post_category' => $item->id]) }}"><i class="fa fa-eye mr-0"></i></a>                                    
                                @endcan
                                ...
                                <a class="badge bg-success mr-2" data-toggle="tooltip" data-placement="top" title="Cập nhật" data-original-title="Edit"
                                    href="{{ route('post_categories.edit',['post_category' => $item->id]) }}"><i class="fa fa-pen mr-0"></i></a>                                    
                                @endcan
                                @can('post_category_delete')
                                <form action="{{ route('post_categories.destroy',['post_category' => $item->id]) }}" method="POST">
                                    @csrf
                                    @method('delete')
    ...
    @can('post_category_delete')
        <script>
            $(document).ready(function () {
                ...
                $("#deleteAllPostCategorySelected").on("click", function () {
                    var ids = [];
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: 'post_categories/massDestroy',
                        data: {
                            ids: ids,
