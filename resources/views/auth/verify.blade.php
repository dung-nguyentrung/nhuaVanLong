                ...
                <div class="card-body">
                    @if (session('resent'))
                        <div class="alert alert-success" role="alert">
                            {{ __('Một đường dẫn xác thực đã được gửi đến mail của bạn.') }}
                        </div>
                    @endif
                    ...
                    <form class="d-inline" method="POST" action="{{ route('verification.resend') }}">
                        @csrf
                        <button type="submit" class="btn btn-primary p-0 m-0 align-baseline">{{ __('bấm vào đây để tạo yêu cầu khác') }}</button>.
                    </form>
                </div>
            </div>
        </div>
